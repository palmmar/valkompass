    ...
    private static Projection Project(Context ctx, Scratch s, int[] sample, double[] shock)
    {
        ...
        for (var k = 0; k < ctx.MunicipalityCount; k++)
        {
            ...
            for (var p = 0; p < slots; p++)
            {
                ...
                // Stöten skalas som standardfelet för en andel, sqrt(p(1-p)). Ett stort parti
                // kan slå fel med fler procentenheter än ett litet, men inte proportionellt
                // fler - båda de enklare varianterna gav sneda intervall i backtestet.
                var noise = p < shock.Length
                    ? Math.Sqrt(Math.Max(0, baseline * (1 - baseline))) * shock[p]
                    : 0;
